...
/**
  ...
  * Author: becca
  * @brief          : this file contains function that handle the 6x mode of the driver
  * 						(drives the 3 phase, 6 step motor behavior manually
  * 						- if you're using 3x mode you can just send high signals and not configure deadtime
...
void phases_start(void)
{
  ...
  //don't delete --------------------------------------------------------------------- (test board pin setup)
  /*
...
/**
  * @brief: this function take in the current step from the hall sensor (in a 3 bit format) and bases the next motor position
  * 				on the current hall sensor reading. currently set up for 6x mode but may be able to simplify more
  * 				since I think theoretically some of this should be configured on the STM
  *
  * 				More info about the bldc phases can be found here:
  * 							-https://media.monolithicpower.com/document/Brushless_DC_Motor_Fundamentals.pdf
  * 							-(I like page 13, 16, and 17)
  */
