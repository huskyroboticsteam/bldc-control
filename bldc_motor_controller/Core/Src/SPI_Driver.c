 ...
 *
 * Usage — This is file that interacts with the DRV8323 driver, main communication protocol is SPI
 *
...
/**
 * @brief  Build all five register shadows
 *
 * Register details:
 *
 *  DRV_CTRL (0x02):
...
DRV8323_Result_t DRV8323_Init(DRV8323_Handle_t *hdrv)
{
    ...
    // Allow DRV8323 to complete power-on reset
    HAL_Delay(5);

    /*
     * Write GATE_HS first to unlock the register file since the DRV8323 ignores
     * writes to any register until LOCK = 0b011.
     * (the TI reference firmware writes GATE_HS twice)
     */
    ...
    // Write remaining control registers
    ...
    // Read status to confirm no active faults
 ...
 * this hardware dead-time both apply. - TODO: choose which one to go with
 */
...
void Motor_InitAll(void)
{
      ...
      g_motor[0].cs_port     = GPIOA;  //TODO: update this lol
    ...
    // https://controllerstech.com/how-to-use-spi-with-stm32/ TODO: what did i mean by this..
