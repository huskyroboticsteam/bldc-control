...
/**
 * @file    hall_sensor.c
 * @brief   file to process hall sensor data from motor and send to phases OR driver
 * 				- send to driver if in 1x mode OR to phases.c to process to be sent to driver
...
/**
  * @brief: reads the hall state directly from the motor and bitshifts to fit 3 bit into 8 bit int
  */
...
//maps hall sensor output to step config (can be changed since not all motors output hall readings in the same order
